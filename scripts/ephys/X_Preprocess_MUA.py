...
### Load and pre-process data: MUA version
...
for session in sessions_subset:
    try:
        ...
        for ch, channel in enumerate(flatten_probe):
            ...
            # Extract chunk for all channels
            chunk_data = reshaped_data_T[:, 35500000:35600000]

            # Compute channel medians
            #chunk_channel_medians = np.median(chunk_data, 1)
            #chunk_channel_offsets = np.median(chunk_data, 1)

            # Subtract channel offsets (median - 32768)
            #chunk_mean_Front = np.mean(chunk_data[:64, :], 0)
            #chunk_mean_Back = np.mean(chunk_data[64:, :], 0)
            chunk_median_Front = np.median(chunk_data[:64, :], 0)
            chunk_median_Back = np.median(chunk_data[64:, :], 0)
            #chunk_median = np.median(chunk_data, 0)

            #plt.plot(chunk_mean)
            #plt.show()
        
            # Extract data chunk for single channel
            channel_data = reshaped_data_T[channel, 35500000:35600000]
            #channel_data = channel_data - chunk_median
            #channel_data = channel_data - chunk_mean
            if(channel < 64):
                channel_data = channel_data - chunk_median_Front
            else:
                channel_data = channel_data - chunk_median_Back
            ...
            plt.plot(channel_data_MUA_bandpass + (35 *ch))
